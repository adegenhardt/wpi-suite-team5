...
public class CalendarSidebar extends JPanel {
	...
	/**
	 * Constructor method for the CalendarSidebar class.
	 * It prepares a split pane in the Calendar module that displays
	 * the table of events and the host of graphical components that enable
	 * the creation of categories and the filtering of events via category
	 * selection.
	 */
	private CalendarSidebar() {
		 ...
		 * window for applied filters in the CalendarSidebar.
		 * TODO Add a button for removing only the selected categories from the
		 * Applied Filters text window TODO Have Current categories drop down
		 * build from only not already a filter categories
		 */
		...
		// Creation of a panel within the scroll pane that holds the Filter/Category manager.
		...
		btnCatCreate.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				...
				// The new category does not exist. build, add to model, and inform user.
				/*
	...
	/**
	 * Populate the category drop down menu that is contained within
	 * the CalendarSidebar split pane.
	 */
	...
	public void populateTable() {
		...
		// selected. This should never happen, but if it does, this is the warning.
		...
		for (int i = 0; i < numRows; i++) {
			for (int j = 0; j < 4; j++) {
				...
				// Populate the first column of the table of upcoming events in the 
				...
				// Populate the second column of the table of upcoming events in the
				// CalendarSidebar with the starting dates for each of the upcoming
				// events.
				if (j == 1) {
					try {
				...
				// Populate the third column of the table of upcoming events in the
				// CalendarSidebar with the ending dates for each of the upcoming
				// events.
				if (j == 2) {
					try {
				...
				// Populate the fourth column of the table of upcoming events in the
				// CalendarSidebar with the descriptions for each of the upcoming
				// events.
				if (j == 3) {
					try {
	...
	/**
	 * Overrides the paintComponent method to retrieve the events 
	 * and categories on the first painting.
	 * 
	...
	/**
	 * Method for setting the contents of the table of upcoming events
	 * in the CalendarSidebar.
	 * 
	 * @param eventTable A table of events that is received as input.
	 */
	 ...
	 *  
	 * @param commitmentTable A table of commitments that is received as input.
	 */
